    ...
    for(epoch=0; epoch<10000; epoch++)  //训练次数
    {
        ...
        for(p=binary_dim-1; p>=0 ; p--)
        {
            ...
            for(j=0; j<hidenode; j++) 
            {
                ...
                // std::cout << "F_delta[j] = " << F_delta[j] << endl;
                // std::cout << "I_future_delta[j] = " << I_future_delta[j] << endl;
                ...
                // std::cout << "+u_i[" << j << "] = " << U_I[j][j] << ": " << alpha <<
                //   " * " << I_delta[j] << " * " << h_pre[j] << "\n";
                // print_double_arr("h_pre: ", h_pre, hidenode);
